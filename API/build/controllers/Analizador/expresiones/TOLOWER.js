...
class Tolower extends EXPRESION_1.Expresion {
    ...
    getValor(tree, table) {
        ...
        if (valor1.Tipo.getTipos() === TIPO_1.tipos.CADENA) {
            let nueva = new PRIMITIVO_1.default(new TIPO_1.default(TIPO_1.tipos.CADENA), valor1.valor.toLowerCase(), this.linea, this.columna);
            table.update(this.exp.ID, nueva);
            return nueva;
        }
        else {
            tree.newERROR("SEMANTICO", "TOLOWER ES SOLO PARA VARIABLES TIPO STRING", this.linea, this.columna);
